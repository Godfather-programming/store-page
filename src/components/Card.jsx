...
<span className={styles.list} onClick={clickHandler}> <Link to={`/products/${id}`}> <TbListDetails size="1.5rem" color="#FF5722"/> </Link>  </span> 
   ...
   <span className={styles.list} onClick={clickHandler}> <Link to={`/products/${id}`}> <TbShoppingBagCheck size="1.5rem" color="#FF5722"/> </Link>  </span> 
  {buy ?  <div className={styles.amount}> {number === 1 ? <span className={`${styles.sign} ${styles.trash}`} onClick={removeHandler}> <LuTrash /> </span>  : <span className={styles.sign} onClick={decreaseHandler}> - </span> } <span> {number} </span> <span className={styles.sign} onClick={increaseHandler}> + </span> </div> : <span className={styles.buy} onClick={buyHandler}> <TbShoppingBagCheck size="1.5rem" color="#fff"/> </span>}  
   </div> */}
